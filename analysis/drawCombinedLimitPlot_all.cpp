...
int main( int argc, char* argv[] ) {
  ...
  std::string limitsFile_only13_hadr( Form( "%s/limits_w0p014_comb_only13_hadr.txt", dir.c_str() ));
  ...
  TGraph* gr_only13_hadr_obs = new TGraph(0);
  TGraph* gr_only13_hadr_exp = new TGraph(0);
  TGraphAsymmErrors* gr_only13_hadr_exp_1sigma = new TGraphAsymmErrors(0);
  TGraphAsymmErrors* gr_only13_hadr_exp_2sigma = new TGraphAsymmErrors(0);

  gr_only13_hadr_obs       ->SetName("only13_hadr_obs");
  gr_only13_hadr_exp       ->SetName("only13_hadr_exp");
  gr_only13_hadr_exp_1sigma->SetName("only13_hadr_exp_1sigma");
  gr_only13_hadr_exp_2sigma->SetName("only13_hadr_exp_2sigma");
  ...
  getLimitGraphs( limitsFile_only13_hadr, gr_only13_hadr_obs, gr_only13_hadr_exp, gr_only13_hadr_exp_1sigma, gr_only13_hadr_exp_2sigma );
  ...
  gr_only13_hadr_obs->Draw("L same");
  ...
  c1->SaveAs( Form("%s/limit_comb_all_w0p014.eps" , dir.c_str() ));
  c1->SaveAs( Form("%s/limit_comb_all_w0p014.pdf" , dir.c_str() ));

  c1->SetLogx();
  h2_axes->GetXaxis()->SetMoreLogLabels();
  h2_axes->GetXaxis()->SetNoExponent();

  c1->SaveAs( Form("%s/limit_comb_all_w0p014_logx.eps" , dir.c_str() ));
  c1->SaveAs( Form("%s/limit_comb_all_w0p014_logx.pdf" , dir.c_str() ));
  ...
  c1->SaveAs( Form("%s/limit_comb_all_w0p014_bands.eps" , dir.c_str() ));
  c1->SaveAs( Form("%s/limit_comb_all_w0p014_bands.pdf" , dir.c_str() ));

  c1->SetLogx();
  h2_axes->GetXaxis()->SetMoreLogLabels();
  h2_axes->GetXaxis()->SetNoExponent();

  c1->SaveAs( Form("%s/limit_comb_all_w0p014_bands_logx.eps" , dir.c_str() ));
  c1->SaveAs( Form("%s/limit_comb_all_w0p014_bands_logx.pdf" , dir.c_str() ));
...
void getLimitGraphs( const std::string& limitsFile, TGraph* gr_obs, TGraph* gr_exp, TGraphAsymmErrors* gr_exp_1sigma, TGraphAsymmErrors* gr_exp_2sigma ) {
  ...
  TString limitsFile_tstr(limitsFile);
